...
dN_dS_statistics <- data.frame(
  "Species Compared" = c("Human vs Chimp", "Human vs Chimp", "Human vs Chimp", "Human vs Mouse", "Human vs Mouse", "Human vs Mouse"),
  "Gene Group" = c("Genomic", "TF", "bHLH","Genomic", "TF", "bHLH"),
  "Mean dN/dS" = c(Genomic = mean(reducido_dN_dS_chimp$dN_dS_chimp), 
                                        TF = mean(FT_dN_dS_chimp$dN_dS_chimp), 
                                        bHLH = mean(bHLH_dN_dS_chimp$dN_dS_chimp),
                                        Genomic = mean(reducido_dN_dS_mouse$dN_dS_mouse), 
                                        TF = mean(FT_dN_dS_mouse$dN_dS_mouse), 
                                        bHLH = mean(bHLH_dN_dS_mouse$dN_dS_mouse)
                                        ),
                               "Median dN/dS" = c(Genomic = median(reducido_dN_dS_chimp$dN_dS_chimp),
                                          TF = median(FT_dN_dS_chimp$dN_dS_chimp),
                                          bHLH = median(bHLH_dN_dS_chimp$dN_dS_chimp),
                                          Genomic = median(reducido_dN_dS_mouse$dN_dS_mouse),
                                          TF = median(FT_dN_dS_mouse$dN_dS_mouse),
                                          bHLH = median(bHLH_dN_dS_mouse$dN_dS_mouse)
                                          ),
                               "Olig2 Percentile" = c(mean(reducido_dN_dS_chimp$dN_dS_chimp <= dN_dS_olig2_chimp)*100,
                                              mean(FT_dN_dS_chimp$dN_dS_chimp <= dN_dS_olig2_chimp)*100,
                                              mean(bHLH_dN_dS_chimp$dN_dS_chimp <= dN_dS_olig2_chimp)*100,
                                              mean(reducido_dN_dS_mouse$dN_dS_mouse <= dN_dS_olig2_mouse)*100,
                                              mean(FT_dN_dS_mouse$dN_dS_mouse <= dN_dS_olig2_mouse)*100,
                                              mean(bHLH_dN_dS_mouse$dN_dS_mouse <= dN_dS_olig2_mouse)*100
                                              ))
